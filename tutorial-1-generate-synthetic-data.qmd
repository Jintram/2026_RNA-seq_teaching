---
title: "Tutorial: Generate Synthetic RNA-seq Data"
format: html
---
...
Now convert the `df_gene_data_all` to a dataframe with variability
that we'll generate with a negative binomial distribution.

::: {.callout-note}
### Technical note
You might be used to **gaussian variability**, but 
...
noise (approaching Poisson). We'll use $\theta = 100$, which is chosen
such that we'll see some false positives and false negatives in our data.
Sometimes, the term $\theta$ is replaced with $\alpha = 1/\theta$, which is the 
dispersion parameter used by DESeq2.
:::

For example, a gene with an expression of 500, will have a variability of 
`500 + 500^2 / 100 = 3000` (pretty high!), and it's distribution will look as follows:
...
Let's check whether if we would have a 1000 samples, we would indeed find a 
mean of `500` and a variability of `3000`;
...
print(paste0("Mean = ", round(mean(simulated_counts),2)))
print(paste0("Variance = ", round(var(simulated_counts),2)))
```

This part of the theory works!

By the way, the shape of the negative binominal distribution becomes more 
asymmetric when the size factor decreases, e.g. for $\theta=3$:
...
Excellent! In the next section, we'll check which of the differential expression
that we engineered into this data we can find back using RNA-seq analysis by DESeq2.