...
DESeq2 takes a little more sophisticated appraoch. The total counts in a sample can be skewed by gene composition, e.g. a few highly expressed differentially expressed (DE) genes can affect total counts \[[1](https://hbctraining.github.io/Intro-to-DGE/lessons/02_DGE_count_normalization.html)\], \[[2](https://link.springer.com/article/10.1186/gb-2010-11-10-r106)\].
It therefor calculates a pseudo-reference sample, with arithmetic averages
over the samples for each gene, then normalizes each gene by the reference sample, 
and then calculates the median normalized expression per sample. 
This somewhat complex scheme is summarized below:
...
Let's now apply normalization of the count values, and illustrate the effect
with a plot for 10 random genes.
...
In the plot above, the red points better reflect the true transcript concentrations 
compared between the samples.

As a final point, note that the total counts for each sample
are not after DESeq2's normalization.
